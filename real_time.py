...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        # Reshape keypoints array to have 30 frames
        keypoints_sequence = np.array([keypoints] * 30)

        # Prediction logic
        if len(keypoints_sequence) == 30:  # Check if sequence length is 30
            prediction = model.predict(np.expand_dims(keypoints_sequence, axis=0))[0]
            print(actions[np.argmax(prediction)])


        # Visualize predictions
        cv2.putText(image, actions[np.argmax(prediction)], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
